...
# Bootstrap imports for first-time syncs against existing repositories.
#
...
# content. To avoid this, on the very first sync (state contains no
# `github_repository_file.managed` entries) we walk the target repo's default
# branch tree and emit an `imports.tf` containing one import block per
# already-existing managed file. Subsequent runs see the resource in state
# and skip this step entirely. The previous standalone CODEOWNERS resource,
# if present in state from an earlier deploy, is migrated by the module's
# `moved` block and does not need an import here.
...
if(!$repositoryCreationModeEnabled) {
    ...
    $hasManagedInState = $false
    if($stateList -and $stateList[0].success) {
        $stateLogPath = Join-Path $terraformModulePath $stateListLog
        if(Test-Path $stateLogPath) {
            foreach($line in Get-Content -Path $stateLogPath) {
                if($line -match '^module\.github\.github_repository_file\.managed\b') {
                    $hasManagedInState = $true
                    break
                }
            }
        }
    ...
    if($hasManagedInState) {
        Write-Host "github_repository_file.managed already in state for $orgAndRepoName; skipping import bootstrap."
    } else {
        Write-Host "First-time sync detected for $orgAndRepoName; checking the target repo for pre-existing managed files."

        $candidateImportPaths = @($managedFiles.Keys) + ".github/CODEOWNERS"
        $candidateImportPaths = @($candidateImportPaths | Select-Object -Unique)
        ...
        if(!$repoInfo -or !$repoInfo[0].success -or !$repoInfo[0].output.default_branch) {
            ...
            if(!$treeResult -or !$treeResult[0].success -or !$treeResult[0].output.tree) {
                ...
                $importsToWrite = @($candidateImportPaths | Where-Object { $existingFiles -contains $_ })

                if($importsToWrite.Count -eq 0) {
                    Write-Host "No pre-existing managed-file paths found in $orgAndRepoName; no imports needed."
                } else {
                    $importBlocks = New-Object System.Collections.Generic.List[string]
                    $importBlocks.Add("# Auto-generated by Invoke-RepositorySync.ps1 on the first sync of this repo.")
